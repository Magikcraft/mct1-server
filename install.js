/**
 * This file contains code shamelessly, and grateful cribbed from the Medium Phantomjs installer. It is licensed under Apache 2.0.
 */
...
const { downloadUrl } = require('./lib/util')
...
function checkMCT1WorldsLocally() {
    const mct1WorldsExistLocally = fs.existsSync(`${downloadUrl}`)
...
function exit(code) {
    validExit = true
    if (code === 0) {
        console.log(`MCT1 Server installed. Type 'mct1-server start' to start`)
    }
    process.exit(code || 0)
}
...
function copyIntoPlace(extractedPath) {
    return new Promise((resolve, reject) => {
        console.log('Removing', downloadUrl)
        rmrf(downloadUrl, error => {
            if (error) {
                console.log(
                    'Error copying ' + extractedPath + ' to ' + downloadUrl
                )
                reject(error)
            }
